...
#'
estim_chen <- function(y, method = "BFGS", clvl = NULL, n_bootstrap = NULL) {
  y <- as.vector(unlist(y))
  checkmate::assert_numeric(y, lower = 0, any.missing = FALSE)
  checkmate::assert_choice(method, c("Nelder-Mead", "BFGS", "CG", "L-BFGS-B", "SANN", "Brent"))
  ...
  if (!is.null(n_bootstrap)) {
    boot_func <- function(data, indices) { # Function that receives the indices as generated by boot::boot to take a sample from the data
      ...
      suppressWarnings(bootstrap_estim <- stats::optim(par = c(1, 0.3),
                                      fn = log_likelihood,
                                      y = bootstrap_y,
                                      method = method,
                                      hessian = TRUE,
    ...
    if (!is.null(clvl)) {
      ...
      colnames(ci) <- c("lambda_ci", "delta_ci")
      out <- list(t = param,
                  ci = ci)
    } else {
      out <- list(t = param)
