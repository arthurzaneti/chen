...
#' estim_chen_rpr(rchen(100, c(0.7,7)), tau = 0.5, clvl = 0.95)
#' estim_chen_rpr(rchen(100, c(0.7,7)), tau = 0.5, clvl = 0.95, n_bootstrap = 100)
#'
...
#_________________________________________BOOTSTRAP_______________________________________
  if (!is.null(n_bootstrap)) {
    ...
    if (!is.null(clvl)) {
      ...
      boot_ci_mu <- boot::boot.ci(boot_results, type = "norm", index = 2, conf = clvl)
      lambda_ci <- boot_ci_lambda$normal[c(2,3)]
      mu_ci <- boot_ci_mu$normal[c(2,3)]
      ci <- cbind(lambda_ci, mu_ci)
